...
class NotesServices {
    ...
    func getNotes() async -> Result<[NoteItem], CustomMessageError> {
        do {
            ...
            let homeID = try await supabase.from("users").select("homeID").eq("userID", value: userID).execute().data.convertDataToString().convertStringToDictionary()?["homeID"] as? String ?? ""
            let noteDict = try await supabase.from("notes").select("*").eq("homeID", value: homeID).execute().data.convertDataToString().convertStringToDictionaryArray() ?? [[:]]
            var items = [NoteItem]()
    ...
    func createNote(title: String) async -> Result<Void, CustomMessageError> {
        do {
            ...
            try await supabase.from("notes").upsert(dict).execute()
    ...
    func sendNotificationToPartner(title: String, message: String, pushCategory: PushNotificationIdentifiers.Category, notificationSoundString: String, data: [String: Any]? = nil) async {
        do {
    ...
    func connectSocket(noteID: String, completion: @escaping (_ message: RealtimeMessage) -> Void) {
