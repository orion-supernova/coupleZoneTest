...
class SettingsViewController: UIViewController {
    ...
    private func changeUsername(with username: String, completion: @escaping () -> Void) {
        Task {
            ...
            try await SensitiveData.supabase.from("users").update(["username": username]).eq("email", value: userEmail).execute()
    ...
    private func addJoinHomeButtonIfNeeded() {
        Task {
            ...
            let homeDict = try await SensitiveData.supabase.from("homes").select("*", head: false).eq("id", value: homeID).execute().data.convertDataToString().convertStringToDictionary()
            if homeDict == nil {
                let button = UIButton(type: .system)
    ...
    private func changeAnniversaryDate(with dateString: String, completion: @escaping () -> Void) {
        Task {
            ...
            try await SensitiveData.supabase.from("homes").update(["anniversary_date": dateToPut]).eq("id", value: homeID).execute()
    ...
    private func getAnniversaryText() async -> String {
        do {
            let homeID = await getHomeID()
            let anniversaryDict = try await SensitiveData.supabase.from("homes").select("anniversary_date", head: false).eq("id", value: homeID).execute().data.convertDataToString().convertStringToDictionary()
