...
class CreateOrJoinRoomViewController: UIViewController {
    ...
    private func createANewHome(anniversaryDateString: String) {
        Task {
            ...
            try await SensitiveData.supabase.from("homes").upsert(dict).execute()
            let partnerIDs: [String] = [userIDString]
            try await SensitiveData.supabase.from("homes").update(["partnerUserIDs": partnerIDs]).eq("id", value: homeID).execute()
    ...
    private func checkIfUserHasAHome() async -> Bool {
        do {
            ...
            let homeIDString = try await SensitiveData.supabase.from("users").select("homeID").eq("userID", value: userID).execute().data.convertDataToString().convertStringToDictionary()?["homeID"] as? String ?? ""
    ...
    private func checkIfHomeExists(with idString: String) async -> Bool {
        do {
            let homeDict = try await SensitiveData.supabase.from("homes").select("*", head: false).eq("id", value: idString).execute().data.convertDataToString().convertStringToDictionary()
    ...
    private func connectUserWithHome(homeID: String, completion: @escaping (Result<String, CustomMessageError>) -> Void) {
        Task {
            do {
                ...
                try await SensitiveData.supabase.from("homes").update(["partnerUserIDs": partnerIDsOfTheRoom]).eq("id", value: homeID) .execute()
                // Update User's Home ID and Partner ID
                try await SensitiveData.supabase.from("users").update(["homeID": homeID, "partnerUserID": partnerID]).eq("userID", value: userID).execute()
                // Update Partner's Partner ID
                try await SensitiveData.supabase.from("users").update(["partnerUserID": userID]).eq("userID", value: partnerID).execute()
    ...
    private func getPartnerIDsForHome(with homeID: String) async -> [String]? {
        do {
            let homePartnerIDs = try await SensitiveData.supabase.from("homes").select("partnerUserIDs").eq("id", value: homeID).execute().data.convertDataToString().convertStringToDictionary()?["partnerUserIDs"] as? [String]
